# Orchestrator / Worker モデル

🎯 **最重要概念**: agentmineの中核となるアーキテクチャパターン

agentmineは**Orchestrator/Workerモデル**を採用しています。これは「AI as Orchestrator」設計原則を具体化したものです。

---

## 概要

```
┌──────────────────────────────────────────────────────────────────────┐
│                                                                      │
│  Human (ユーザー)                                                    │
│    │ 「3つのタスクを並列で実行して」                                 │
│    ▼                                                                 │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │                    Orchestrator (Claude Code等)                 │ │
│  │                                                                  │ │
│  │  1. タスク分析・計画                                             │ │
│  │  2. agentmine worker run --exec --detach                       │ │
│  │     (worktree作成 + scope適用 + session開始)                    │ │
│  │  3. 完了待ち（worker wait / status）                            │ │
│  │  4. マージ判断・実行（Orchestratorが決定）                       │ │
│  │  5. PR作成                                                       │ │
│  └──────────────────────┬─────────────────────────────────────────┘ │
│                         │                                            │
│          ┌──────────────┼──────────────┐                            │
│          ▼              ▼              ▼                            │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐                   │
│  │   Worker    │ │   Worker    │ │   Worker    │                   │
│  │  (Task #3)  │ │  (Task #4)  │ │  (Task #5)  │                   │
│  │             │ │             │ │             │                   │
│  │ worktree-3/ │ │ worktree-4/ │ │ worktree-5/ │                   │
│  │ scope適用済 │ │ scope適用済 │ │ scope適用済 │                   │
│  │             │ │             │ │             │                   │
│  │ ※agentmine │ │ ※agentmine │ │ ※agentmine │                   │
│  │   非使用    │ │   非使用    │ │   非使用    │                   │
│  └──────┬──────┘ └──────┬──────┘ └──────┬──────┘                   │
│         │              │              │                             │
│         └──────────────┴──────────────┘                             │
│                        │ (exit code / signal)                       │
│                        ▼                                             │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │                   agentmine (Blackboard)                        │ │
│  │                                                                  │ │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────────┐                  │ │
│  │  │  Tasks   │  │ Sessions │  │ Memory Bank  │                  │ │
│  │  │  状態    │  │  履歴    │  │  決定事項    │                  │ │
│  │  └──────────┘  └──────────┘  └──────────────┘                  │ │
│  │                                                                  │ │
│  │  Orchestratorのみがアクセス（Workerは非使用）                    │ │
│  └────────────────────────────────────────────────────────────────┘ │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
```

---

## 役割定義

### Orchestrator（オーケストレーター）

**定義**: 並列実行を計画・監視するAIクライアント、または人間

**典型例**:
- Claude Code
- Codex
- Gemini CLI
- 人間（手動実行）

**責務**:
1. **計画**: タスクを分析し、並列実行戦略を決定
2. **起動**: `agentmine worker run` でWorkerを起動
3. **監視**: Worker状態を監視（`worker status` / `wait`）
4. **DoD検証**: lint/test/build を実行
5. **マージ判断**: コンフリクト解決、マージ実行
6. **PR作成**: 完成したコードをPRにまとめる

**やらないこと**:
- ❌ コード作成（それはWorkerの仕事）
- ❌ worktree内での作業

### Worker（ワーカー）

**定義**: 隔離されたworktree内でコードを書くAIエージェント

**特徴**:
- 専用worktreeで作業（`.agentmine/worktrees/task-<id>/`）
- スコープ制御適用済み
- agentmineにアクセスしない（完全隔離）
- 自動承認モードで実行

**責務**:
1. **コード作成**: タスクに従ってコードを書く
2. **テスト追加**: 必要なテストを作成
3. **コミット**: 変更をコミット

**やらないこと**:
- ❌ タスク管理
- ❌ 他のWorkerとの通信
- ❌ マージ判断
- ❌ agentmine CLI実行

### agentmine

**定義**: Blackboard（共有データ領域）兼実行基盤

**責務**:
1. **データ管理**: DB（tasks, sessions, agents, memories）
2. **worktree管理**: 作成・削除・スコープ適用
3. **Worker起動**: AIクライアント実行
4. **セッション記録**: exit code, artifacts, duration
5. **Memory Bank提供**: プロジェクト決定事項の注入

**やらないこと**:
- ❌ 並列実行の計画
- ❌ マージ判断
- ❌ リトライ判断
- ❌ コード作成

---

## 責務分担の詳細

### Orchestratorの責務

#### 1. 並列実行の計画

```typescript
// Orchestratorの思考例（擬似コード）

// ユーザーから「task #3, #4, #5を並列で」と依頼

// 1. タスク情報取得
const tasks = await exec('agentmine task list --json');

// 2. 依存関係分析
if (task3.dependsOn.includes(task4.id)) {
  // task4を先に実行
  await exec('agentmine worker run 4 --exec');
  await exec('agentmine worker done 4');
}

// 3. 並列起動
await Promise.all([
  exec('agentmine worker run 3 --exec --detach'),
  exec('agentmine worker run 5 --exec --detach'),
]);
```

#### 2. Worker監視

```bash
# Orchestratorが実行するコマンド

# 状態確認
agentmine worker status

# 完了待ち
agentmine worker wait 3 5

# エラー確認
agentmine session list --status failed
```

#### 3. DoD検証

```bash
# Orchestratorが各worktreeでDoD検証

cd .agentmine/worktrees/task-3
pnpm lint && pnpm test && pnpm build

# 成功 → マージ
# 失敗 → Worker再実行 or 人間に通知
```

#### 4. マージ判断

```bash
# Orchestratorが判断してマージ実行

# コンフリクトなし → 自動マージ
git merge task-3-s101

# コンフリクトあり → Orchestratorが解決
git merge task-3-s101  # conflict
# ... Orchestratorがコンフリクト解決 ...
git commit
```

### Workerの責務

#### 1. コード作成

Workerは与えられたプロンプト（promptContent + Memory Bank + タスク）に従ってコードを書く。

**Workerが受け取る情報**:
```markdown
## Memory Bank Summary
- ルール: バグ修正時は必ず回帰テストを追加
- 規約: コミット形式はConventional Commits
- アーキテクチャ: DBはPostgreSQL（本番）/ SQLite（ローカル）

## Project Context (Memory Bank)
The following project context files are available:
- .agentmine/memory/architecture/database-selection.md
- .agentmine/memory/tooling/test-framework.md

## タスク
**ログイン機能を実装**
POST /api/login でJWTトークンを返すAPIを実装してください。
```

#### 2. スコープ遵守

Workerは物理的にアクセスできるファイルのみ編集：
- `src/` - write可能
- `tests/` - write可能
- `docs/` - read-only
- `.env` - 存在しない（excluded）

### agentmineの責務

#### 1. worktree作成

```bash
# agentmineが内部で実行（Orchestratorは意識しない）

git worktree add .agentmine/worktrees/task-3 -b task-3-s123
cd .agentmine/worktrees/task-3
git sparse-checkout set src/ tests/ package.json
# excludeパターン（.env等）を自動除外
```

#### 2. Worker起動

```bash
# agentmine worker run の内部動作

# 1. セッション開始（DBに記録）
# 2. worktree作成
# 3. スコープ適用
# 4. Memory Bank出力
# 5. promptContent生成
# 6. AIクライアント起動
claude-code exec --dangerously-skip-permissions "$PROMPT"
```

#### 3. セッション記録

```typescript
// Worker終了時にagentmineが記録

await db.update(sessions)
  .set({
    status: 'completed',
    exitCode: process.exitCode,
    completedAt: new Date(),
    artifacts: changedFiles, // git diffから自動収集
  })
  .where(eq(sessions.id, sessionId));
```

---

## 通信フロー

### Orchestrator → agentmine

```
┌─────────────────┐
│  Orchestrator   │
└────────┬────────┘
         │ agentmine worker run 3 --exec --detach
         ▼
┌─────────────────┐
│   agentmine     │
│   - worktree作成 │
│   - Worker起動   │
└────────┬────────┘
         │ sessionId, pid, worktreePath
         ▼
┌─────────────────┐
│  Orchestrator   │
│  (監視継続)      │
└─────────────────┘
```

### Worker → agentmine

**重要**: Workerはagentmineにアクセスしない

```
┌─────────────────┐
│     Worker      │
│  (隔離環境)      │
│   ↓             │
│  コード作成      │
│   ↓             │
│  git commit     │
│   ↓             │
│  exit           │
└────────┬────────┘
         │ exit code
         ▼
┌─────────────────┐
│   agentmine     │
│  (exit code記録) │
└─────────────────┘
```

---

## 典型的な実行フロー

### シーケンス図

```
Orchestrator                      agentmine                Git / Workers
 │                                  │                           │
 │  1. タスク取得                   │                           │
 │ ─────────────────────────────────>│                           │
 │  agentmine task list --json      │                           │
 │                                   │                           │
 │  2. Worker起動（並列）            │                           │
 │ ─────────────────────────────────>│                           │
 │  agentmine worker run 3 --exec --detach                      │
 │                                   │  git worktree add ...     │
 │                                   │  git sparse-checkout ...  │
 │                                   │───────────────────────────>│
 │                                   │                     (作業)│
 │                                   │                           │
 │  3. Worker完了待ち                │                           │
 │ ─────────────────────────────────>│                           │
 │  agentmine worker wait 3          │                           │
 │                                   │                           │
 │  4. DoD検証                        │                           │
 │ ──────────────────────────────────────────────────────────────>│
 │  cd worktree && pnpm test         │                           │
 │                                   │                           │
 │  5. マージ判断・実行                │                           │
 │ ──────────────────────────────────────────────────────────────>│
 │  git merge task-3-s123             │                           │
 │                                   │                           │
 │  6. 完了処理                       │                           │
 │ ─────────────────────────────────>│                           │
 │  agentmine worker done 3          │                           │
```

---

## 利点

### 1. 責務分離

- Orchestrator: 計画・判断
- Worker: コード作成
- agentmine: 実行基盤

→ それぞれが専念できる

### 2. 柔軟性

Orchestratorは状況に応じて戦略を変更可能：
- 並列数の調整
- 優先順位の変更
- リトライ戦略

### 3. スケーラビリティ

Workerを増やすことで並列度を向上：
```bash
# 4つ並列
agentmine worker run 1 --exec --detach
agentmine worker run 2 --exec --detach
agentmine worker run 3 --exec --detach
agentmine worker run 4 --exec --detach
```

### 4. 安全性

Worker隔離により：
- 互いに干渉しない
- スコープ制御で意図しない変更を防止

---

## よくある質問

### Q1: Workerは他のWorkerと通信できますか？

**A**: いいえ、できません。Workerは完全に隔離されています。調整が必要な場合はOrchestratorが行います。

### Q2: Workerが失敗したらどうなりますか？

**A**: agentmineがセッションを`failed`に記録し、Orchestratorに通知します。リトライ判断はOrchestratorが行います。

### Q3: Orchestratorは人間でもよいですか？

**A**: はい。人間がCLIを手動で実行してもOKです。ただし、AIが自動で判断・実行する方が効率的です。

### Q4: Web UIはどの役割ですか？

**A**: Web UIは主に人間用のインターフェースです。タスク管理・Agent定義・Worker監視を行いますが、Orchestrator的な判断はしません。

---

## 関連ドキュメント

- @../02-architecture/design-principles.md - AI as Orchestrator原則
- @../07-runtime/worker-lifecycle.md - Worker起動から終了まで
- @../07-runtime/parallel-execution.md - 並列実行の詳細
- @./scope-control.md - スコープ制御の仕組み
- @../appendix/glossary.md - 用語定義
