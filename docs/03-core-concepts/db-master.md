# DBマスター方式

🎯 **Single Source of Truth**: すべてのデータはDBで管理する

agentmineは**DBマスター方式**を採用しています。これは「Single Source of Truth」設計原則の具体的な実装です。

---

## 原則

**すべてのデータ（タスク、Agent、Memory、設定）はDBで管理する。ファイルはスナップショット/エクスポート用のみ。**

```
┌─────────────────────────────────────────────────────────────────┐
│  Single Source of Truth = DB                                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Web UI ──┐                                                      │
│  CLI ─────┼──→ DB (マスター) ──→ Worker用スナップショット出力   │
│  MCP ─────┘                                                      │
│                                                                 │
│  .agentmine/ は .gitignore（リポジトリには含めない）            │
└─────────────────────────────────────────────────────────────────┘
```

---

## 理由

### 1. データ整合性

**問題**: ファイルベースだと同じ情報が複数箇所に存在し、更新漏れが発生

```
❌ ファイルベース（従来）:
.agentmine/
├── agents/coder.yaml        # Agent定義
├── prompts/coder.md         # promptContent
└── settings.yaml            # 設定

→ coder.yamlを更新したが、prompts/coder.mdの更新を忘れた
→ 古い情報と新しい情報が混在
```

```
✅ DBマスター（agentmine）:
agents テーブル
├── name: "coder"
├── prompt_content: "..."
└── version: 3

→ 1箇所更新すれば全て最新
→ 整合性が保証される
```

### 2. リアルタイム共有

チーム全員が常に最新データを参照：

```typescript
// Web UIで編集
await agentService.updateAgent('coder', { model: 'opus' });

// 即座にCLIで反映
$ agentmine agent show coder
model: opus  ← 最新
```

### 3. 変更履歴

DBで履歴テーブルを管理：

```typescript
// agentHistory テーブル
{
  agentId: 1,
  version: 2,
  snapshot: { /* 変更前の状態 */ },
  changedBy: "user-123",
  changedAt: "2026-01-20T10:00:00Z",
  changeSummary: "modelをsonnetからopusに変更"
}
```

→ いつ、誰が、何を変更したかトレース可能

### 4. 検索・フィルタリング

SQLで効率的なクエリ：

```typescript
// Memory Bankから特定カテゴリを検索
const memories = await db
  .select()
  .from(memories)
  .where(
    and(
      eq(memories.category, 'architecture'),
      eq(memories.status, 'active')
    )
  );
```

ファイルベースでは`grep`や全ファイル読み込みが必要。

---

## DB管理されるデータ

| データ | テーブル | スナップショット出力 | 用途 |
|--------|---------|---------------------|------|
| **タスク** | `tasks` | - | タスク管理 |
| **セッション** | `sessions` | - | 実行履歴 |
| **Agent定義** | `agents` | `.agentmine/agents/*.yaml` | バックアップ・移行 |
| **Memory Bank** | `memories` | `worktree/.agentmine/memory/` | Worker参照 |
| **設定** | `settings` | `.agentmine/config.yaml` | バックアップ・移行 |
| **監査ログ** | `audit_logs` | - | 監査 |
| **Agent履歴** | `agent_history` | - | 変更履歴 |
| **Memory履歴** | `memory_history` | - | 変更履歴 |

---

## データフロー

### 作成・更新フロー

```
┌─────────────┐
│  Web UI     │ ──┐
└─────────────┘   │
                  │ 1. 作成/更新
┌─────────────┐   │
│  CLI        │ ──┤
└─────────────┘   │
                  │
┌─────────────┐   │
│  MCP        │ ──┘
└─────────────┘
                  ▼
            ┌──────────┐
            │    DB    │ ← Single Source of Truth
            └────┬─────┘
                 │
                 │ 2. 必要時にスナップショット出力
                 ▼
         ┌───────────────┐
         │ .agentmine/   │
         │ ├── agents/   │ ← エクスポート・バックアップ用
         │ ├── memory/   │ ← エクスポート・バックアップ用
         │ └── config.yaml│ ← エクスポート・バックアップ用
         └───────────────┘
```

### Worker実行時フロー

```
Worker起動時のみファイル出力:

┌──────────┐
│    DB    │
└────┬─────┘
     │
     │ agentmine worker run 5
     │
     ├─→ Agent定義取得 → promptContent生成
     ├─→ Memory Bank取得 → .agentmine/memory/ 出力
     └─→ 設定取得 → スコープ適用

         ▼
   worktree/.agentmine/memory/
   ├── architecture/
   │   └── database-selection.md
   └── tooling/
       └── test-framework.md
```

---

## スナップショット vs DB

### スナップショット（ファイル）の用途

✅ **用途**:
- バックアップ
- 他プロジェクトへの設定コピー
- 移行（プロジェクト間）
- Worker参照（read-only）
- Git管理（必要な場合）

❌ **非用途**:
- リアルタイム編集（DBで行う）
- 真実源（あくまでスナップショット）

### DB（マスター）の用途

✅ **用途**:
- すべての作成・更新・削除
- リアルタイム共有
- 検索・フィルタリング
- 変更履歴管理
- トランザクション

---

## スナップショット出力

### 手動エクスポート

```bash
# 全データエクスポート
agentmine export --output ./backup/

# 個別エクスポート
agentmine agent export coder --output ./coder.yaml
agentmine memory export --output ./memory/
```

出力結果:
```
./backup/
├── agents/
│   ├── coder.yaml
│   └── reviewer.yaml
├── memory/
│   ├── architecture/
│   │   └── database-selection.md
│   └── tooling/
│       └── test-framework.md
└── config.yaml
```

### 自動出力（Worker起動時）

```bash
agentmine worker run 5
```

agentmineが自動的に：
1. Memory Bankを取得
2. `worktree/.agentmine/memory/` に出力（read-only）
3. Worker起動

---

## Git連携

### .gitignore戦略

```gitignore
# .gitignore

.agentmine/
!.agentmine/.gitkeep
```

**理由**:
- DBがマスターなので、スナップショットはリポジトリ不要
- 必要ならエクスポート先を別ディレクトリに指定

### Git管理が必要な場合

```bash
# エクスポート先を変更
agentmine memory export --output ./docs/memory/

# Git管理
git add docs/memory/
git commit -m "docs: Memory Bank更新"
```

---

## インポート（移行用）

既存YAMLファイルからDBへインポート：

```bash
# 既存の.agentmine/からDBへ
agentmine db import --from .agentmine/

# 個別インポート
agentmine agent import --file ./coder.yaml
agentmine memory import --dir ./memory/
```

**用途**:
- 旧バージョンからの移行
- 他プロジェクトからの設定コピー
- バックアップからの復元

---

## DB戦略

### PostgreSQL（メイン）

**用途**: チーム開発・本番環境

**メリット**:
- 複数人での同時アクセス
- トランザクション・排他制御
- pgvectorによるベクトル検索（将来）
- 変更履歴・監査ログの一元管理

**接続**:
```bash
export AGENTMINE_DB_URL="postgresql://user:pass@localhost:5432/agentmine"
agentmine init
```

### SQLite（サブ）

**用途**: 個人開発・ローカル環境

**メリット**:
- ゼロ設定（`agentmine init`で即使用可能）
- ファイルベースで簡単
- オフライン環境

**接続**:
```bash
# デフォルト
agentmine init
# → .agentmine/data.db が作成される
```

🔗 @../04-data/overview.md - DB戦略の詳細

---

## 実装例

### Agent定義の作成

```typescript
// CLI/Web UI/MCPから呼ばれる
import { agentService } from '@agentmine/core';

// DBに保存
const agent = await agentService.createAgent({
  name: 'coder',
  client: 'claude-code',
  model: 'sonnet',
  scope: {
    read: ['**/*'],
    write: ['src/**', 'tests/**'],
    exclude: ['**/*.env'],
  },
  promptContent: `
# Coder Agent

コーディング規約に従ってコードを作成してください。
  `,
});

// → agents テーブルに保存
// → agent_history テーブルにバージョン記録
```

### Memory Bankの作成

```typescript
import { memoryService } from '@agentmine/core';

// DBに保存
const memory = await memoryService.createMemory({
  id: 'database-selection',
  category: 'architecture',
  title: 'データベース選定',
  summary: 'PostgreSQL（本番）/ SQLite（ローカル）',
  status: 'active',
  content: `
# データベース選定

## 決定
PostgreSQL（本番）、SQLite（ローカル）

## 理由
- pgvectorによるAI機能の親和性
- SQLiteはゼロ設定でローカル開発に最適
  `,
});

// → memories テーブルに保存
// → memory_history テーブルにバージョン記録
```

### Worker起動時のスナップショット出力

```typescript
// agentmine worker run 内部処理

import { memoryService } from '@agentmine/core';
import fs from 'fs/promises';
import path from 'path';

async function exportMemoryToWorktree(worktreePath: string) {
  // 1. DBから取得
  const memories = await memoryService.listMemories({
    status: 'active'
  });

  // 2. ファイル出力
  const outputDir = path.join(worktreePath, '.agentmine', 'memory');
  await fs.mkdir(outputDir, { recursive: true });

  for (const memory of memories) {
    const categoryDir = path.join(outputDir, memory.category);
    await fs.mkdir(categoryDir, { recursive: true });

    await fs.writeFile(
      path.join(categoryDir, `${memory.id}.md`),
      memoryService.toMarkdown(memory)
    );
  }

  // 3. read-onlyに設定
  await exec(`chmod -R a-w ${outputDir}`);
}
```

---

## メリット・デメリット

### メリット

✅ **データ整合性**: 1箇所更新で全て最新
✅ **リアルタイム共有**: チーム全員が同じデータ
✅ **変更履歴**: いつ誰が何を変更したかトレース
✅ **検索効率**: SQL/ORMで高速クエリ
✅ **トランザクション**: 複数変更の原子性保証

### デメリット

⚠️ **DB必須**: DBなしでは動作しない（SQLiteで軽減）
⚠️ **移行コスト**: 既存ファイルベースからの移行が必要
⚠️ **Git Diff**: スナップショットをGit管理しないと差分が見えにくい

---

## よくある質問

### Q1: ファイルを直接編集したら反映されますか？

**A**: いいえ。ファイルはスナップショットなので、編集しても反映されません。DBを更新してください。

```bash
# ❌ ファイル編集（反映されない）
vim .agentmine/agents/coder.yaml

# ✅ DBを更新（正しい）
agentmine agent update coder --model opus
```

### Q2: スナップショットをGit管理すべきですか？

**A**: 基本的には不要です。DBがマスターなので。ただし、以下の場合はGit管理を検討：
- 変更履歴を可視化したい
- レビュープロセスに含めたい
- バックアップとして残したい

その場合はエクスポート先を別ディレクトリに：
```bash
agentmine memory export --output ./docs/memory/
git add docs/memory/
```

### Q3: DBが壊れたらどうなりますか？

**A**: 定期的にエクスポートしてバックアップを取ってください：
```bash
# 定期バックアップ
agentmine export --output ./backup/$(date +%Y%m%d)/

# 復元
agentmine db import --from ./backup/20260120/
```

### Q4: オフラインで作業できますか？

**A**: SQLiteを使えばオフライン作業可能です：
```bash
export AGENTMINE_DB_URL="file:.agentmine/data.db"
agentmine init
```

---

## 関連ドキュメント

- @../02-architecture/design-principles.md - Single Source of Truth原則
- @../04-data/overview.md - DB戦略（PostgreSQL + SQLite）
- @../04-data/schema.md - DBスキーマ定義
- @../04-data/export-import.md - エクスポート・インポート
- @../10-decisions/002-sqlite-default.md - DB選定のADR
