# スコープ制御

🎯 **安全性の要**: Workerがアクセスできるファイルを物理的に制限する仕組み

agentmineは**スコープ制御**により、Workerがアクセスできるファイルを物理的に制限します。これにより、AIに自動承認モードで安全に作業させることができます。

---

## 原則

**exclude（除外） → read（参照） → write（編集）の優先順位で、Worker作業環境を物理的に制限する。**

```
┌──────────────────────────────────────────────────────────────┐
│  スコープ制御の階層                                          │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  【最優先】exclude                                            │
│    → sparse-checkoutで物理的に除外（存在しない）             │
│                                                              │
│  【次優先】read                                               │
│    → worktreeに存在、参照可能                                 │
│                                                              │
│  【最後】write                                                │
│    → 明示的に指定されたファイルのみ編集可能                    │
│    → それ以外は chmod a-w で読み取り専用                     │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```

---

## なぜスコープ制御が必要か

### 1. 意図しない変更の防止

Workerが誤って重要なファイルを変更するのを防ぐ：

```
❌ スコープ制御なし:
Worker「タスク: ログイン機能実装」
→ うっかり.envファイルを変更
→ 本番APIキーが漏洩

✅ スコープ制御あり:
Worker「タスク: ログイン機能実装」
→ .envは存在しない（exclude）
→ 変更不可能
```

### 2. 自動承認モードの実現

物理的に制限されているため、AIに自動承認で作業させられる：

```bash
# スコープ制御なし → 人間の承認が必要
claude-code exec "ログイン機能実装"
# 「.envを編集しますか？」→ 人間が判断

# スコープ制御あり → 自動承認で実行可能
claude-code exec --dangerously-skip-permissions "ログイン機能実装"
# .envは存在しないので、編集される心配なし
```

### 3. 並列実行の安全性

複数のWorkerが互いに干渉しない：

```
Worker A（タスク: 認証API）
scope: src/auth/

Worker B（タスク: 決済API）
scope: src/payment/

→ 互いに干渉しない（物理的に分離）
```

---

## スコープフィールド

### フィールド定義

| フィールド | 説明 | 物理的実装 |
|-----------|------|-----------|
| `exclude` | アクセス不可 | sparse-checkoutで除外（存在しない） |
| `read` | 参照可能 | worktreeに存在 |
| `write` | 編集可能 | 明示的に指定されたファイルのみ |
| （暗黙） | read - write - exclude | chmod a-w で読み取り専用 |

### 優先順位

```
exclude → read → write
```

**評価ロジック**:
1. **exclude にマッチ → 除外**（sparse-checkout）
2. exclude でない && **read にマッチ → 参照可能**（存在）
3. exclude でない && read にマッチ && **write にマッチ → 編集可能**
4. exclude でない && read にマッチ && **write に非マッチ → 読み取り専用**（chmod a-w）

---

## Agent定義例

```yaml
name: coder
client: claude-code
model: sonnet
scope:
  # 優先順位: exclude → read → write

  exclude:                 # 最優先: sparse-checkoutで物理的に除外
    - "**/*.env"          # すべての.envファイル
    - "**/secrets/**"      # secretsディレクトリ配下すべて
    - "**/.aws/**"         # AWS認証情報
    - "**/node_modules/**" # 依存ライブラリ

  read:                    # 参照可能
    - "**/*"               # 全ファイル（excludeを除く）

  write:                   # 編集可能（明示的に指定）
    - "src/**"             # srcディレクトリ配下
    - "tests/**"           # testsディレクトリ配下
    - "package.json"       # package.json
```

---

## 物理的実装

### 1. Git Sparse Checkout（exclude）

**目的**: exclude にマッチするファイルを物理的に除外

```bash
cd .agentmine/worktrees/task-5

# sparse-checkout初期化
git sparse-checkout init --cone

# excludeパターンを除外しつつ、readパターンを含める
git sparse-checkout set --no-cone '/*' '!**/*.env' '!**/secrets/**'

# 結果: .envファイルは物理的に存在しない
ls -la
# → .env が存在しない
```

**効果**:
- Worker側でファイルが見えない
- 編集しようとしてもファイルが存在しない
- AIが誤って変更することが不可能

### 2. chmod（read-only化）

**目的**: write に含まれないファイルを読み取り専用化

```bash
cd .agentmine/worktrees/task-5

# write対象外のファイルを読み取り専用に
find . -type f \
  ! -path './src/*' \
  ! -path './tests/*' \
  ! -name 'package.json' \
  -exec chmod a-w {} \;

# 結果: docs/配下は読み取り専用
ls -la docs/
# → -r--r--r-- (書き込み権限なし)
```

**効果**:
- Workerは参照可能
- 編集しようとするとPermission Denied
- AIが誤って変更しにくい

---

## Worker起動時の適用フロー

```
┌─────────────────────────────────────────────────────────────┐
│  agentmine worker run 5 --exec                              │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
         ┌──────────────────────┐
         │ 1. Git Worktree作成  │
         │  .agentmine/         │
         │  worktrees/task-5/   │
         └──────────┬───────────┘
                    │
                    ▼
         ┌──────────────────────┐
         │ 2. DB からAgent定義  │
         │  取得（scope含む）   │
         └──────────┬───────────┘
                    │
                    ▼
         ┌──────────────────────┐
         │ 3. Sparse Checkout   │
         │  exclude適用         │
         │  （物理的に除外）     │
         └──────────┬───────────┘
                    │
                    ▼
         ┌──────────────────────┐
         │ 4. chmod適用         │
         │  write対象外を       │
         │  読み取り専用化       │
         └──────────┬───────────┘
                    │
                    ▼
         ┌──────────────────────┐
         │ 5. Worker AI起動     │
         │  自動承認モード       │
         └──────────┬───────────┘
                    │
                    ▼
         ┌──────────────────────┐
         │ 6. コード作成・編集   │
         │  （スコープ内のみ）   │
         └──────────┬───────────┘
                    │
                    ▼
         ┌──────────────────────┐
         │ 7. Worker完了        │
         └──────────┬───────────┘
                    │
                    ▼
         ┌──────────────────────┐
         │ 8. 事後チェック       │
         │  git diff で検証     │
         └──────────────────────┘
```

---

## 事後チェック（検証）

スコープ制御は物理的制限だが、100%防げるわけではない。事後チェックで検証する。

### チェック内容

```typescript
// Worker完了後、agentmineが自動実行

// 1. 変更されたファイル一覧取得
const changedFiles = await exec('git diff --name-only HEAD');

// 2. スコープ違反チェック
const violations = changedFiles.filter(file => {
  // excludeにマッチ → 違反
  if (matchesExclude(file, agent.scope.exclude)) {
    return true;
  }

  // writeに含まれない → 違反
  if (!matchesWrite(file, agent.scope.write)) {
    return true;
  }

  return false;
});

// 3. 違反があればエラー
if (violations.length > 0) {
  throw new ScopeViolationError(violations);
}
```

### 違反時の動作

```bash
# Worker完了
agentmine worker done 5

# スコープ違反検出
Error: Scope violation detected
- .env (excluded)
- docs/architecture.md (not in write scope)

# セッションを failed に記録
# Orchestratorに通知
```

---

## セキュリティ上の考慮事項

### 攻撃ベクトルと対策

| 攻撃ベクトル | スコープ制御の効果 | 事後チェック |
|-------------|-------------------|-------------|
| `chmod u+w` で書き込み権限復元 | 防げない | 検出可能 |
| `git sparse-checkout disable` | 防げない | 検出可能 |
| 直接ファイル作成（echo > .env） | 防げない | 検出可能 |
| シンボリックリンクで迂回 | 防げない | 検出可能 |

### 多層防御

```
┌─────────────────────────────────────────────────────────────┐
│  レイヤー1: sparse-checkout + chmod                          │
│    → 誤操作・うっかりミスを防ぐ                              │
│    → AIが「自然に」守る                                      │
├─────────────────────────────────────────────────────────────┤
│  レイヤー2: 事後チェック（git diff）                         │
│    → 意図的な回避を検出                                      │
│    → 悪意あるAgent・AIを検出                                 │
├─────────────────────────────────────────────────────────────┤
│  レイヤー3: Orchestratorによる検証                           │
│    → DoD検証（lint, test, build）                           │
│    → コードレビュー                                          │
└─────────────────────────────────────────────────────────────┘
```

---

## worktree例

### スコープ適用前

```
.agentmine/worktrees/task-5/
├── .env                    # 存在する
├── .aws/
│   └── credentials         # 存在する
├── src/
│   ├── auth.ts
│   └── payment.ts
├── tests/
│   └── auth.test.ts
├── docs/
│   └── architecture.md
├── package.json
└── README.md
```

### スコープ適用後

```yaml
scope:
  exclude:
    - "**/*.env"
    - "**/.aws/**"
  read:
    - "**/*"
  write:
    - "src/**"
    - "tests/**"
    - "package.json"
```

```
.agentmine/worktrees/task-5/
# .env → 存在しない（sparse-checkout除外）
# .aws/ → 存在しない（sparse-checkout除外）
├── src/                    # write可能
│   ├── auth.ts
│   └── payment.ts
├── tests/                  # write可能
│   └── auth.test.ts
├── docs/                   # read-only (chmod a-w)
│   └── architecture.md     # -r--r--r--
├── package.json            # write可能
└── README.md               # read-only (chmod a-w)
```

---

## 実装例

### スコープ適用（TypeScript）

```typescript
import { exec } from 'node:child_process';
import { promisify } from 'node:util';
import minimatch from 'minimatch';

const execAsync = promisify(exec);

async function applyScopeControl(
  worktreePath: string,
  scope: AgentScope
) {
  // 1. sparse-checkout適用
  await applySparseCheckout(worktreePath, scope);

  // 2. chmod適用（write対象外を読み取り専用化）
  await applyChmod(worktreePath, scope);
}

async function applySparseCheckout(
  worktreePath: string,
  scope: AgentScope
) {
  const cwd = worktreePath;

  // sparse-checkout初期化
  await execAsync('git sparse-checkout init --cone', { cwd });

  // excludeパターンを否定形に変換
  const negatedPatterns = scope.exclude.map(p => `!${p}`);

  // readパターンとexcludeの否定形を組み合わせ
  const patterns = [...scope.read, ...negatedPatterns];

  // sparse-checkout設定
  await execAsync(
    `git sparse-checkout set --no-cone ${patterns.join(' ')}`,
    { cwd }
  );
}

async function applyChmod(
  worktreePath: string,
  scope: AgentScope
) {
  // write対象外のファイル一覧取得
  const { stdout } = await execAsync('git ls-files', { cwd: worktreePath });
  const files = stdout.trim().split('\n');

  for (const file of files) {
    // writeにマッチしない → 読み取り専用化
    const isWritable = scope.write.some(pattern =>
      minimatch(file, pattern, { dot: true })
    );

    if (!isWritable) {
      const filePath = path.join(worktreePath, file);

      if (process.platform === 'win32') {
        await execAsync(`attrib +R "${filePath}"`);
      } else {
        await execAsync(`chmod a-w "${filePath}"`);
      }
    }
  }
}
```

### 事後チェック（TypeScript）

```typescript
async function verifyScopeCompliance(
  worktreePath: string,
  scope: AgentScope
): Promise<string[]> {
  const cwd = worktreePath;

  // 変更されたファイル一覧取得
  const { stdout } = await execAsync('git diff --name-only HEAD', { cwd });
  const changedFiles = stdout.trim().split('\n').filter(Boolean);

  const violations: string[] = [];

  for (const file of changedFiles) {
    // excludeにマッチ → 違反
    if (scope.exclude.some(p => minimatch(file, p, { dot: true }))) {
      violations.push(`${file} (excluded)`);
      continue;
    }

    // writeに含まれない → 違反
    if (!scope.write.some(p => minimatch(file, p, { dot: true }))) {
      violations.push(`${file} (not in write scope)`);
    }
  }

  return violations;
}
```

---

## 制限事項

### 1. 絶対的な防御ではない

物理的制限は「誤操作防止」が主目的。悪意ある回避は事後チェックで検出。

### 2. パフォーマンス影響

sparse-checkout + chmod は大規模リポジトリで遅くなる可能性。

### 3. Windows環境

chmod の代わりに attrib を使用。完全な互換性はない。

---

## よくある質問

### Q1: Worker が chmod u+w で書き込み権限を復元したら？

**A**: 事後チェックで検出されます。agentmineがWorker完了時に`git diff`で変更を確認し、write範囲外の変更があればエラーとして記録します。

### Q2: Worker が git sparse-checkout disable したら？

**A**: 同様に事後チェックで検出されます。exclude対象のファイルが変更されていれば違反として扱われます。

### Q3: スコープを厳しくし過ぎると作業できないのでは？

**A**: はい。スコープは適切に設定する必要があります。タスクの性質に応じて、Agent定義を使い分けることを推奨します：
- `coder`: 広い範囲で作業（src/, tests/）
- `reviewer`: 読み取り専用（write: []）
- `docs-writer`: docs/のみ編集可能

### Q4: read にも write にも含まれないファイルは？

**A**: sparse-checkoutで除外されます（存在しない）。read に含めない場合、Workerは参照すらできません。

---

## 関連ドキュメント

- @../02-architecture/design-principles.md - Isolation & Safety原則
- @../05-features/worktree-scope.md - 実装詳細
- @../05-features/agent-definition.md - Agent定義のscope設定
- @../07-runtime/worker-lifecycle.md - Worker起動フロー
- @../appendix/glossary.md - 用語定義（scope）
