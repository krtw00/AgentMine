...
## Memory Governance（記憶ガバナンス）

記憶の増加に伴う品質劣化を防ぐため、信頼度・有効期限・承認の管理を導入する。
...
1. `active = true` の記憶のみ
2. `expires_at` が未来または null
3. `confidence >= 0.3`（低信頼記憶を除外）
4. タスクの `write_scope` とタグがマッチするものを優先
5. `confidence * relevance_score` の降順でソート
6. 上限N件（設定可能、デフォルト10件）

### API変更

#### PATCH /api/projects/:id/memories/:memoryId/approve

記憶を人間が承認する。confidence を 1.0 に引き上げ、approved_by と approved_at を記録する。
