      ...
      <table className="w-full text-xs" style={{ borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ background: "#202020" }}>
            ...
            <th
              className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap"
              style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", minWidth: 200 }}
            >
              タスク / 実行
            </th>
            <th
              className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap"
              style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 100 }}
            >
              エージェント
            </th>
            <th
            ...
            <th
              ...
              style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 200 }}
            >
              <div>タイムライン</div>
              {timelineData && (
          ...
          {runs.length === 0 ? (
            ...
            runs.map((run) => {
              ...
              const width = timelineData
                ? Math.max(1, (((run.finishedAt ? new Date(run.finishedAt).getTime() : Date.now()) - new Date(run.startedAt).getTime()) / timelineData.range) * 100)
                : 0;
              return (
                <tr
                  key={run.id}
                ...
                >
                  ...
                  <td className="px-2.5 py-1.5 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                    <div
                      ...
                      style={{
                        background: "linear-gradient(to right, rgba(255,255,255,0.06) 1px, transparent 1px), rgba(255,255,255,0.04)",
