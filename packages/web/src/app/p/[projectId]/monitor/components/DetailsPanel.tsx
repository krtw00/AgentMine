  ...
  return (
    <div className="flex flex-col" style={{ width: 520, minWidth: 420 }}>
      ...
      <div className="flex-1 overflow-auto p-2.5" style={{ background: "#1b1b1b" }}>
        ...
        {activeTab === "git" && (
          <div className="grid gap-2 text-xs" style={{ gridTemplateColumns: "140px 1fr" }}>
            <div className="text-[#a0a0a0]">ヘッドSHA</div><div className="font-mono">{run.headSha || "-"}</div>
            <div className="text-[#a0a0a0]">未コミット変更</div><div className="font-mono">{run.worktreeDirty === null ? "不明" : run.worktreeDirty ? "あり" : "なし"}</div>
            <div className="text-[#a0a0a0]">ブランチ</div><div className="font-mono">{run.branchName}</div>
            <div className="text-[#a0a0a0]">ワークツリーパス</div><div className="font-mono break-all">{run.worktreePath}</div>
          </div>
        )}
      </div>
    </div>
