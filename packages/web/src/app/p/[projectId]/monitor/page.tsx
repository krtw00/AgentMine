...
function LiveTerminalGrid({ runs, runOutputs }: { runs: ExtendedRun[]; runOutputs: Map<number, OutputLine[]> }) {
  // 実行中 + 最近出力があったRunを表示
...
function LiveTerminalPane({ run, lines }: { run: ExtendedRun; lines: OutputLine[] }) {
  ...
  const statusColor = isRunning ? "#cca700" : run.status === "completed" ? "#89d185" : run.status === "failed" ? "#f14c4c" : "#a0a0a0";

  return (
    <div className="rounded-lg border overflow-hidden flex flex-col" style={{ background: "#0d1117", borderColor: "#30363d", maxHeight: 320 }}>
      {/* ペインヘッダー */}
      <div className="flex items-center gap-2 px-2.5 py-1.5 border-b" style={{ background: "#161b22", borderColor: "#30363d" }}>
        <span className="w-2 h-2 rounded-full" style={{ background: statusColor, boxShadow: isRunning ? `0 0 6px ${statusColor}` : "none" }} />
        <span className="text-[11px] font-semibold" style={{ color: "#c9d1d9" }}>
          実行 #{run.id}
        </span>
        <span className="text-[10px] px-1.5 py-0.5 rounded border" style={{ color: "#8b949e", background: "#21262d", borderColor: "#30363d" }}>
          {run.taskTitle}
        </span>
      ...
      >
        {lines.length === 0 ? (
          ...
          lines.slice(-100).map((line, i) => {
            if (line.type === "system") {
              return <div key={i} style={{ color: "#58a6ff" }}>--- {line.data}</div>;
            }
            if (line.type === "stderr") {
              return <div key={i} style={{ color: "#f85149" }}>{line.data}</div>;
            }
            if (line.type === "exit") {
              const c = line.exitCode === 0 ? "#7ee787" : "#f85149";
              return <div key={i} style={{ color: c }}>EXIT code={line.exitCode}</div>;
            }
                  ...
                  if (parsed) {
                    return (
                      <div key={j} className="flex gap-1">
                        <span className="shrink-0" style={{ color: parsed.color, opacity: 0.6 }}>{parsed.label}</span>
                        <span style={{ color: parsed.color, wordBreak: "break-all" }}>{parsed.text.slice(0, 300)}</span>
                      </div>
                    );
                  }
                  return jl.trim() ? <div key={j} style={{ color: "#c9d1d9" }}>{jl.slice(0, 300)}</div> : null;
                })}
              </div>
            );
          })
...
export default function MonitorPage() {
  ...
  const countRunsForTask = useCallback((taskId: number): number => {
  ...
  const overviewSvg = useMemo(() => {
    ...
    const sorted = [...filteredRuns].sort((a, b) => new Date(a.startedAt).getTime() - new Date(b.startedAt).getTime());
      ...
      <div key={task.id}>
        ...
        >
          <span
            ...
            onClick={(e) => { e.stopPropagation(); if (hasChildren) toggleTaskCollapse(task.id); }}
          >
    ...
    <div className="h-full flex flex-col" style={{ background: "#1e1e1e", color: "#d4d4d4", fontFamily: "ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, sans-serif" }}>
      ...
      <div className="px-3 py-2 border-b flex items-center gap-3 flex-wrap" style={{ background: "#252526", borderColor: "#3c3c3c" }}>
        ...
        <label className="flex items-center gap-2 text-xs text-[#a0a0a0]">
          <input type="checkbox" checked={includeDescendants} onChange={(e) => setIncludeDescendants(e.target.checked)} style={{ accentColor: "#0e639c" }} />
          子孫を含む
        </label>
        <button
      ...
      {overviewSvg && (
        <div className="mx-3 mt-2 rounded-lg overflow-hidden border" style={{ background: "#111", borderColor: "#3c3c3c" }}>
          <div className="flex items-center justify-between px-2.5 py-1.5 text-[11px] border-b" style={{ color: "#a0a0a0", borderColor: "rgba(255,255,255,0.06)" }}>
            ...
            <span className="font-mono">{overviewSvg.ticks[0]} - {overviewSvg.ticks[4]}</span>
          </div>
          <svg
            viewBox="0 0 1000 64"
          ...
          <div className="flex justify-between px-2.5 py-1 text-[10px]" style={{ color: "#a0a0a0" }}>
            {overviewSvg.ticks.map((t, i) => <span key={i}>{t}</span>)}
          </div>
        </div>
      )}
      ...
      <div className={`flex-1 flex overflow-hidden ${detailsHidden ? "" : ""}`} style={{ minHeight: 0 }}>
        ...
        <div className="flex-1 flex" style={{ borderRight: detailsHidden ? "none" : "1px solid #3c3c3c", minWidth: 0 }}>
          ...
          <div className="w-64 overflow-auto p-2.5 border-r" style={{ background: "#1b1b1b", borderColor: "#3c3c3c" }}>
            ...
            >
              ...
              <span className="ml-auto text-[10px] text-[#a0a0a0] font-mono">{allRuns?.length ?? 0} 件</span>
            </div>
            ...
            <table className="w-full text-xs" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "#202020" }}>
                  <th className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap" style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 90 }}>ステータス</th>
                  <th className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap" style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", minWidth: 200 }}>タスク / 実行</th>
                  <th className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap" style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 100 }}>エージェント</th>
                  <th className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap" style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 70 }}>開始</th>
                  <th className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap" style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 70 }}>所要時間</th>
                  <th className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap" style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 70 }}>完了定義</th>
                  <th className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b whitespace-nowrap" style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 60 }}>違反</th>
                  <th className="sticky top-0 z-10 text-left px-2.5 py-2 font-medium border-b" style={{ color: "#a0a0a0", borderColor: "#3c3c3c", background: "#202020", width: 200 }}>
                    <div>タイムライン</div>
                    {timelineData && (
                      <div className="flex justify-between text-[10px] font-normal mt-1" style={{ color: "#666" }}>
                        {[0, 0.25, 0.5, 0.75, 1].map((p, i) => (
                          <span key={i}>{formatTime(new Date(timelineData.minTime + timelineData.range * p).toISOString())}</span>
                        ))}
                      </div>
                    )}
                  </th>
              ...
              <tbody>
                {filteredRuns.length === 0 ? (
                  ...
                  filteredRuns.map((run) => {
                    ...
                    const left = timelineData ? ((new Date(run.startedAt).getTime() - timelineData.minTime) / timelineData.range) * 100 : 0;
                    const width = timelineData ? Math.max(1, (((run.finishedAt ? new Date(run.finishedAt).getTime() : Date.now()) - new Date(run.startedAt).getTime()) / timelineData.range) * 100) : 0;
                    return (
                      <tr
                        key={run.id}
                      ...
                      >
                        ...
                        <td className="px-2.5 py-1.5 border-b" style={{ borderColor: "rgba(255,255,255,0.06)", color: "#a0a0a0" }}>
                          {formatTime(run.startedAt)}
                        </td>
                        <td className="px-2.5 py-1.5 border-b font-mono" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                          {formatDuration(run.startedAt, run.finishedAt)}
                        </td>
                        <td className="px-2.5 py-1.5 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                        ...
                        <td className="px-2.5 py-1.5 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                          <div className="relative h-3.5 rounded" style={{ background: "linear-gradient(to right, rgba(255,255,255,0.06) 1px, transparent 1px), rgba(255,255,255,0.04)", backgroundSize: "25% 100%" }}>
                            <div
        ...
        {!detailsHidden && selectedRun && (
          <div className="flex flex-col" style={{ width: 520, minWidth: 420 }}>
            {/* Details Header */}
            <div className="px-3 py-2 border-b flex items-center gap-2" style={{ background: "#252526", borderColor: "#3c3c3c" }}>
              <h2 className="text-[13px] font-semibold m-0">詳細</h2>
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 text-xs rounded-full border" style={{ background: "#2d2d2d", borderColor: "#3c3c3c" }}>
                <span className="text-[#a0a0a0]">選択中</span>
                <strong className="text-[#d4d4d4] font-semibold">実行 #{selectedRun.id}</strong>
              </span>
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 text-xs rounded-full border font-mono" style={{ background: "#2d2d2d", borderColor: "#3c3c3c" }}>
                <span className="text-[#a0a0a0]">head_sha</span>
                <strong className="text-[#d4d4d4]">{selectedRun.headSha?.slice(0, 7) || "-"}</strong>
              </span>
            ...
            {/* Tabs */}
            <div className="flex gap-2 px-2.5 py-2 border-b" style={{ background: "#252526", borderColor: "#3c3c3c" }}>
              {tabs.map((tab) => (
                <button
                  key={tab.id}
            ...
            <div className="flex-1 overflow-auto p-2.5" style={{ background: "#1b1b1b" }}>
              ...
              {activeTab === "timing" && (
                <div className="grid gap-2 text-xs" style={{ gridTemplateColumns: "140px 1fr" }}>
                  ...
                  <div className="text-[#a0a0a0]">ランナー実行</div><div className="font-mono">{formatDuration(selectedRun.startedAt, selectedRun.finishedAt)}</div>
              ...
              {activeTab === "violations" && (
                <div className="grid gap-2 text-xs" style={{ gridTemplateColumns: "140px 1fr" }}>
                  <div className="text-[#a0a0a0]">件数</div><div className="font-mono">{selectedRun.scopeViolationCount || 0}</div>
              ...
              {activeTab === "git" && (
                <div className="grid gap-2 text-xs" style={{ gridTemplateColumns: "140px 1fr" }}>
                  <div className="text-[#a0a0a0]">ヘッドSHA</div><div className="font-mono">{selectedRun.headSha || "-"}</div>
                  <div className="text-[#a0a0a0]">未コミット変更</div><div className="font-mono">{selectedRun.worktreeDirty === null ? "不明" : selectedRun.worktreeDirty ? "あり" : "なし"}</div>
                  <div className="text-[#a0a0a0]">ブランチ</div><div className="font-mono">{selectedRun.branchName}</div>
                  <div className="text-[#a0a0a0]">ワークツリーパス</div><div className="font-mono break-all">{selectedRun.worktreePath}</div>
                </div>
              )}
            </div>
          </div>
      ...
      >
        ...
        <form onSubmit={handleCreateTask} className="flex-1 overflow-auto p-3" style={{ background: "#1b1b1b" }}>
          <div className="mb-3">
